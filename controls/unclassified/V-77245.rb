...
syntax matches the letter case of the actual filename on the system."
  tag fix: "Ensure the following mitigations are turned \"ON\" for
plugin-container.exe:
...
\"Options:\".  It is recommended the file be in a read-only network location."
  describe "Check Ensure Exploit Protection system-level mitigation to validate plugin-container.exe is set to DEP is ON;
  ...
  Payload EnableRopSimExec is ON by running Get-ProcessMitigation -Name plugin-container.exe in PowerShell Command"  do
